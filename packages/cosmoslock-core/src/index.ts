...
function randomId(prefix = 'id'): string {
  ...
  if (c?.getRandomValues) { const b = new Uint8Array(16); c.getRandomValues(b); return `${prefix}_${Array.from(b).map(x=>x.toString(16).padStart(2,'0')).join('')}`; }
...
function sha256(msg:string):string {
  ...
  for(let cs=0;cs<words.length;cs+=64){
    for(let i=0;i<16;i++){const o=cs+i*4;w[i]=((words[o]<<24)|(words[o+1]<<16)|(words[o+2]<<8)|words[o+3])>>>0;}
    ...
    for(let i=0;i<64;i++){const S1=(rotr(e,6)^rotr(e,11)^rotr(e,25))>>>0;const ch=((e&f)^(~e&g))>>>0;const t1=(hh+S1+ch+K[i]+w[i])>>>0;const S0=(rotr(a,2)^rotr(a,13)^rotr(a,22))>>>0;const maj=((a&b)^(a&c)^(b&c))>>>0;const t2=(S0+maj)>>>0;hh=g;g=f;f=e;e=(d+t1)>>>0;d=c;c=b;b=a;a=(t1+t2)>>>0;}
    h[0]=(h[0]+a)>>>0;h[1]=(h[1]+b)>>>0;h[2]=(h[2]+c)>>>0;h[3]=(h[3]+d)>>>0;h[4]=(h[4]+e)>>>0;h[5]=(h[5]+f)>>>0;h[6]=(h[6]+g)>>>0;h[7]=(h[7]+hh)>>>0;
  }
  return h.map(x=>x.toString(16).padStart(8,'0')).join('');
}

export function hashBlob(blob:Omit<ServerFragmentBlob,'fragmentHash'>):string{return sha256(JSON.stringify(blob));}

export function protectData(input:{plaintext:string;splitMode:SplitMode;decoyRatio?:number;label?:string}) {
  ...
  const noHash:Omit<ServerFragmentBlob,'fragmentHash'>={version:'0.2',contentId,splitMode:input.splitMode,grid,fragments:arrangement.map(({coordinate,elementId,value,isDecoy})=>({coordinate,elementId,value,isDecoy})),createdAt,...(input.label?{label:input.label}:{})};
...
export function validateIntegrity(blob:ServerFragmentBlob):ValidationResult {
  ...
  return hashBlob(rest)===fragmentHash?{valid:true}:{valid:false,reason:'Fragment hash mismatch.'};
}
