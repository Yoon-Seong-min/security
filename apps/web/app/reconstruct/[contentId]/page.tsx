...
export default function ReconstructPage({ params }: { params: Promise<{ contentId: string }> }) {
  ...
  const go = async (source:'indexeddb'|'file') => {
    ...
    try {
      ...
      let record;
      if (source==='indexeddb') { record=await loadFromIndexedDB(contentId); if (!record) throw new Error('Coordinate map not found in local vault'); }
      else { if (!importedFile) throw new Error('No vault file imported'); record=importedFile; }
      ...
      const msg=e.message??'Unknown error'; setError(msg);
    ...
    <div className="space-y-10 max-w-2xl">
      ...
      <div className="bg-cosmos-surface border border-cosmos-border rounded-xl p-7 space-y-5">
        <div>
          ...
          <input type="password" value={passphrase} onChange={e=>setPassphrase(e.target.value)} placeholder={t('reconstructPassPlaceholder')}
        ...
        <button onClick={()=>go('indexeddb')} disabled={loading||!passphrase}
          ...
          {loading&&sourceUsed==='indexeddb'?<div className="w-5 h-5 border-2 border-cosmos-accent border-t-transparent rounded-full animate-spin"/>:<span className="text-cosmos-accent text-lg">→</span>}
        </button>
        <div className="border border-cosmos-border rounded-xl p-5 space-y-3">
          ...
          {importedFile&&<div className="text-sm font-mono text-cosmos-success">✓ OK</div>}
      ...
      {error&&<div className="bg-red-900/20 border border-red-500/30 rounded-xl p-5"><div className="text-lg font-semibold text-cosmos-danger">{t('reconstructFailed')}</div><div className="text-base font-mono text-cosmos-dim mt-1">{error}</div></div>}
      {plaintext&&(
